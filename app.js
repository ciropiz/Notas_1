...
function resizeCanvas(){
  ...
  canvas.height = canvas.clientWidth / 4; // aspect ratio 4:1
  if(currentMidi) drawNoteAt(currentMidi);
  else drawStaff(currentClef);
}
...
function computeStaffTop() {
  ...
  staffTop = (canvas.height - staffHeight) / 2;
}

/* -------------------- Posiciones predefinidas (nota a línea) -------------------- */
const NOTE_POS = {
...
function handleAnswer(chosen, button){
  ...
  if(actual===chosen){
    ...
    newNote(); // nueva nota solo al acertar
  } else{
...
/* -------------------- Generación notas -------------------- */
const MIN_MIDI=36, MAX_MIDI=83;
function randomMidiInRange(min=MIN_MIDI,max=MAX_MIDI){ return Math.floor(Math.random()*(max-min+1))+min; }

function generateRandomForClef(clef){
  if(clef==='treble'){
    const min=Math.max(MIN_MIDI,noteNameToMidi('C4'));
    const max=Math.min(MAX_MIDI,noteNameToMidi('A5'));
    return randomMidiInRange(min,max);
  } else{
    const min=Math.max(MIN_MIDI,noteNameToMidi('E2'));
    const max=Math.min(MAX_MIDI,noteNameToMidi('C4'));
    return randomMidiInRange(min,max);
  }
}
...
/* -------------------- UI Eventos -------------------- */
function newNote(){
  currentMidi = generateRandomForClef(currentClef);
  drawNoteAt(currentMidi);
